...
def neo4j_query(cypher: str, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    ...
    - Si Neo4jGraph de LangChain está disponible, se usa graph.query (más fácil de extender).
...
def build_ppt_generation_prompt(
    ...
    usando:
      - un pliego de referencia (ref_data),
      - capítulos y extractos adicionales relevantes (extra_caps / extra_extractos).
    """
    ...
    if extra_caps:
        ...
        for exp, caps in caps_by_exp.items():
            ...
            for c in caps[:3]:
                ...
                total_caps += 1
                if total_caps >= 15:  # límite global
                    break
            if total_caps >= 15:
                break
        extra_caps_text = "\n".join(lines)

    # --- 4. Extractos relevantes globales (normativa, solvencia, etc.) ---
    extra_ext_text = ""
    if extra_extractos:
    ...
    # --- Mensaje de usuario (contexto + instrucciones) ---
    user_msg = f"""
...
def generate_ppt_from_graph(user_request: str) -> Dict[str, Any]:
    """
    Versión sin streaming, por si quieres usarla en scripts.
    """
...
async def handle_generate_ppt(question: str):
    """
    Maneja en Chainlit el flujo de generación de PPT con streaming.
    Las evidencias (pliego base, capítulos y extractos) se guardan en metadata y en un Step,
    pero NO se muestran como mensaje en el chat.
    """
    await cl.Message(
    ...
    # 1) Embedding de la petición (async)
    ...
    # 2) Contrato de referencia con PPT
    ...
    # 3) Contexto adicional global (capítulos + extractos relevantes)
    ...
    # 5) Construir evidencias del pliego base y contexto
    ...
    # 5.a) Step para ver el contexto de referencia en el panel de debug
    ...
    # 5.b) Guardar evidencias como metadata OCULTA del mensaje del pliego generado
    msg.metadata = {
        "evidence": {
...
async def on_message(message: cl.Message):
    ...
    try:
        ...
        # 13) Adjuntar evidencias como metadata OCULTA en el mensaje de respuesta
        reply_msg.metadata = {
            "evidence": {

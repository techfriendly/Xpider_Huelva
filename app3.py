...
def neo4j_query(cypher: str, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    ...
    - Si Neo4jGraph de LangChain está disponible, se usa graph.query.
    - Si no, se usa el driver clásico.
    """
    if params is None:
        params = {}
...
def find_reference_ppt_contract(question_embedding: List[float],
    ...
    # Filtrar a aquellos que tengan DocumentoRAG tipo 'PPT'
    for c in candidatos:
...
def get_ppt_reference_data(contract_id: str) -> Optional[Dict[str, Any]]:
    ...
    Devuelve datos de referencia para el PPT de un contrato:
      ...
      - extractos_por_tipo: dict tipo -> lista de textos (recortada)
    """
    # Capítulos ordenados
    ...
    # Limitamos a 5 extractos por tipo
...
# ============================================================
#  PROMPT PARA GENERAR UN PPT (MEJORADO)
# ============================================================
...
def build_ppt_generation_prompt(
    ...
    # 1. Estructura de capítulos del pliego de referencia
    caps_lines = []
    for c in caps_ref:
        if not c["heading"]:
            continue
    ...
    # 2. Extractos del pliego de referencia agrupados por tipo
    ext_lines_ref = []
    ...
    # 3. Capítulos relevantes globales (otros contratos)
    extra_caps_text = ""
    if extra_caps:
    ...
    # 4. Extractos relevantes globales (otros contratos)
    extra_ext_text = ""
    if extra_extractos:
...
# ============================================================
#  GENERACIÓN DE PPT SIN STREAMING (por si quieres usarlo aparte)
# ============================================================
...
# ============================================================
#  GENERACIÓN DE PPT CON STREAMING (Chainlit)
# ============================================================
...
async def handle_generate_ppt(question: str):
    ...
    # Preguntas de seguimiento específicas para el PPT
    ...
    if followups:
        ...
        await cl.Message(
            content="Si quieres, podemos seguir trabajando sobre este pliego:",
            actions=actions,
        ).send()
...
def generate_followup_questions(
    ...
    answer: str,
    max_q: int = 3,
    mode: str = "qa",  # "qa" o "ppt"
) -> List[str]:
    """
    Usa el LLM para proponer hasta max_q preguntas de seguimiento útiles.
    mode="qa": respuestas normales.
    mode="ppt": sugerencias tras generar un PPT.
    """
    ...
    if mode == "ppt":
        extra_instr = (
            "El usuario acaba de recibir un borrador de Pliego de Prescripciones Técnicas. "
            "Sugiere preguntas para refinar, ampliar o adaptar ese pliego."
        )
    else:
        extra_instr = (
            "Sugiere preguntas para profundizar o complementar la información obtenida."
        )

    system_msg = (
        "Eres un asistente que diseña preguntas de seguimiento útiles para técnicos de contratación. "
        f"{extra_instr}"
    )
...
Tu respuesta generada:
\"\"\"{answer}\"\"\"

Tarea:
- Propón hasta {max_q} preguntas de seguimiento útiles y concretas.
- Cada pregunta debe poder responderse usando el mismo grafo de contratos y pliegos.
- No repitas literalmente la pregunta original.
- Evita preguntas genéricas sin contexto.
- Responde SOLO con las preguntas, cada una en una línea distinta, sin numeración ni texto adicional.
"""
...
async def on_followup_action(action: cl.Action):
    """
    Cuando el usuario pulsa una pregunta sugerida, relanzamos on_message
    como si hubiese escrito esa pregunta.
    """
    payload = action.payload or {}
    followup_question = payload.get("question") or action.label or ""
    if not followup_question:
        return

    fake_message = cl.Message(content=followup_question)
...
async def on_message(message: cl.Message):
    ...
    if (
        ...
        or "pliego tecnico" in lower_q
        or "ppt" in lower_q
    ) and any(
        kw in lower_q
        for kw in ["genera", "generar", "redacta", "redáctame", "elabora", "borrador"]
    ):
    ...
    try:
        ...
        # 6) Evidencias en markdown (solo debug/metadata)
        ...
        # 13) Preguntas de seguimiento como botones
        followups = generate_followup_questions(question, answer, max_q=3, mode="qa")
        if followups:
            actions = [
                cl.Action(
                    name="followup",
            ...
            await cl.Message(
                content="Si quieres seguir profundizando, puedes elegir una de estas preguntas:",
                actions=actions,
            ).send()
